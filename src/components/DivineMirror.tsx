...
export interface DivineMirrorState {
  status: StatusType;
  identity: string;
  patent: string;
}

export interface DivineMirrorProps {
  state: DivineMirrorState;
  onSnap: () => void;
}

export function DivineMirror({ state, onSnap }: DivineMirrorProps) {
  ...
  const isScanning = state.status === STATUS.SCANNING;
  const isSnapReady = state.status === STATUS.SNAP_READY;
  ...
  return (
    ...
    >
      {isScanning && <div className="mirror-scan" />}
      ...
      <div className="app-ui">
        ...
        <div className="app-pau-row">
          <button
            ...
            style={{ borderColor: state.identity }}
          >
        ...
        <p className="app-legal">{state.patent}</p>
      </div>
    </div>
  );
}
