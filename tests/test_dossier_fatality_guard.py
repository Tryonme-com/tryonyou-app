...
class TestDossierFatalityGuard(unittest.TestCase):
    ...
    def test_european_thousands_format_reaches_450k_cents(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            evidence = self._evidence(
                Path(tmp),
                {
    ...
    def test_tuesday_0800_without_flag_or_evidence_stays_pending(self) -> None:
        decision = evaluate_dossier_fatality(
            {},
            now=datetime(2026, 8, 18, 8, 0, tzinfo=PARIS),
        )
        self.assertFalse(decision.active)
        self.assertEqual(decision.status, "PENDING_VALIDATION")
