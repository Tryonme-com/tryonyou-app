...
class TestDossierFatalityGuard(unittest.TestCase):
    ...
    def test_cli_now_flag_does_not_confirm_funds_without_evidence(self) -> None:
        from dossier_fatality_guard import main
        from io import StringIO
        from contextlib import redirect_stdout

        buf = StringIO()
        with redirect_stdout(buf):
            code = main(["--now", "2026-05-05T08:00:00+02:00", "--json"])
        payload = json.loads(buf.getvalue())

        self.assertEqual(code, 0)
        self.assertEqual(payload["status"], "PENDING_VALIDATION")
        self.assertFalse(payload["active"])
        self.assertIn("missing_explicit_capital_confirmation_flag", payload["reasons"])
        self.assertIn("missing_evidence_path", payload["reasons"])
