...
class TestSupercommitMaxStaticGuards(unittest.TestCase):
    ...
    def test_env_example_documents_guards_without_secrets(self) -> None:
        example = (ROOT / ".env.example").read_text(encoding="utf-8")

        self.assertIn("TRYONYOU_DEPLOY_BOT_TOKEN=", example)
        self.assertIn("TRYONYOU_CAPITAL_450K_CONFIRMED=", example)
        self.assertIn("DOSSIER_FATALITY_EVIDENCE_PATH=", example)
        self.assertNotIn("```ini", example)
        self.assertIsNone(re.search(r"\d{8,}:[A-Za-z0-9_-]{20,}", example))
        self.assertIsNone(re.search(r'VERCEL_TOKEN="[^"]+"', example))
