...
def _eur_to_cents(raw: Any) -> int | None:
    text = str(raw).strip().replace(" ", "").replace("€", "").replace("EUR", "")
    if not text:
        return None
    if "," in text and "." in text:
        ...
        parts = text.split(",")
        if len(parts) > 1 and all(part.isdigit() for part in parts) and len(parts[-1]) == 3:
            text = "".join(parts)
        else:
            text = text.replace(",", ".")
    elif text.count(".") > 1:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--json", action="store_true", help="Imprime JSON (por defecto ya es JSON).")
