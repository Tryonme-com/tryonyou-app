...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--now", help="Instant ISO-8601 para auditar la ventana (no confirma fondos).")
    parser.add_argument("--json", action="store_true", help="Alias estable: siempre imprime JSON.")
    args = parser.parse_args(argv)

    decision = evaluate_dossier_fatality(now=_parse_now(args.now) if args.now else None)
