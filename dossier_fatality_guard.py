...
No consulta banca por sí solo y nunca confirma fondos reales sin:
- ventana martes 08:00 Europe/Paris,
- flag explícito TRYONYOU_CAPITAL_450K_CONFIRMED,
- evidencia JSON local con importe EUR y referencia bancaria.
...
def _amount_cents(evidence: Mapping[str, Any]) -> int | None:
    ...
    if raw_eur is None:
        return None
    try:
        normalized = str(raw_eur).strip().replace(" ", "").replace("€", "")
        if "," in normalized and "." in normalized:
            ...
            groups = left.split(sep)
            thousands = right.isdigit() and len(right) == 3 and all(part.isdigit() for part in groups)
            if thousands:
                normalized = "".join(groups) + right
            elif sep == ",":
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Valida activación segura del Dossier Fatality.")
    parser.add_argument("--strict", action="store_true", help="Devuelve código 1 si el dossier no está activo.")
    parser.add_argument("--json", action="store_true", help="Imprime JSON.")
    parser.add_argument("--now", help="Instante ISO-8601 para simular la ventana (Europe/Paris).")
    ...
    decision = evaluate_dossier_fatality(now=now)
