...
def _html_index_body() -> bytes | None:
    ...
    # Intentar cargar la plantilla original de la V10
    path = os.path.join(root, "src", "templates", "mirror_v10_final.html")
    if os.path.isfile(path):
        with open(path, encoding="utf-8") as f:
            html = f.read()
        
        # Inyectar los enlaces de Stripe
        html = (
            html.replace("{{STRIPE_LINK_4_5M}}", link_45)
                .replace("{{STRIPE_LINK_98K}}", link_98)
        )
        
        # Añadir un script de transición suave desde el cargador
        transition_script = """
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                console.log('[V10] Protocolo Soberanía Activado');
            });
        </script>
        """
        html = html.replace("</body>", transition_script + "</body>")
