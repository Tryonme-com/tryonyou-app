...
FAST=false
DEPLOY=false
CUSTOM_MESSAGE="chore(supercommit): sincronizar bunker Oberkampf 75011 y galeria web"
...
if [[ "$FAST" != "true" ]]; then
  ...
  if node -e "const s=require('./package.json').scripts||{}; process.exit(s.typecheck?0:1)"; then
    npm run typecheck
  elif node -e "const s=require('./package.json').scripts||{}; process.exit(s.check?0:1)"; then
    npm run check
  fi
  npm run build
fi
...
if [[ -z "$(git status --porcelain)" ]]; then
  ...
  git add -u -- .
  git reset -q -- .env .env.local .env.development.local .env.test.local .env.production.local 2>/dev/null || true

  safe_untracked=()
  while IFS= read -r -d '' path; do
    case "$path" in
      .env|.env.local|.env.*.local|node_modules/*|dist/*|build/*|*.log)
        continue
        ;;
    esac
    safe_untracked+=("$path")
...
if [[ "$DEPLOY" == "true" ]]; then
  ...
  elif npm run | grep -q ' deployall'; then
