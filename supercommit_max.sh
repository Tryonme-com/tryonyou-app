...
if [[ -z "$(git status --porcelain)" ]]; then
  ...
  git add -u -- .
  git reset -q -- .env .env.local .env.development .env.production .env.test dist node_modules build 2>/dev/null || true

  safe_untracked=()
  while IFS= read -r -d '' path; do
    case "$path" in
      .env|.env.local|.env.development|.env.production|.env.test|node_modules/*|dist/*|build/*|*.log)
        continue
        ;;
    esac
    safe_untracked+=("$path")
...
python3 "$ROOT/dossier_fatality_guard.py" || echo "[supercommit_max] Dossier Fatality permanece en validacion."
