...
if [[ -z "$(git status --porcelain)" ]]; then
  ...
  git add -u -- .
  git reset -q -- .env .env.local .env.production .env.development dist node_modules build '*.log' 2>/dev/null || true
  ...
  while IFS= read -r -d '' path; do
    case "$path" in
      .env.example)
        ;;
      .env|.env.local|.env.production|.env.development|node_modules/*|dist/*|build/*|*.log)
        continue
        ;;
    esac
    safe_untracked+=("$path")
