...
- **Vite proxies `/api` to `http://127.0.0.1:8000`.** If you need API endpoints locally, start the FastAPI backend: `uvicorn backend.omega_core:app --reload --port 8000`. The frontend renders independently without the backend.
